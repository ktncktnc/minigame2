var GameBoard = cc.TMXTiledMap.extend({
   ...
   ctor :function (){
    ...
    startGame: function(){
        gameScreen.state = MW.GAME_STATE.PLAY;
        this.schedule(this.checkCollison, 0.25, cc.REPEAT_FOREVER, 0);
    ...
    checkHp: function(){
        ...
        for(var i =0; i < MW.CONTAINER.ENEMIES.length; i++){
            enemy = MW.CONTAINER.ENEMIES[i];
            if(!enemy.active) continue;
            for(var j = 0; j < MW.CONTAINER.SWORDS.length; j++){
                sword = MW.CONTAINER.SWORDS[j];
    ...
    createBarriers: function(){
        var barrier;
        var free_pos = [...Array((MW.MAPSIZE[0] - 2)*(MW.MAPSIZE[1] - 2)).keys()];
        for(var i = 0; i < MW.BARRIER_SIZE[0]; i++){
            barrier = Barrier.getOrCreateBarrier();
            var rand = [];
            rand.push(free_pos[getRandomInt(0, free_pos.length)]);

            if((rand[0] % (MW.MAPSIZE[0] - 2)) != 0)
                rand.push(rand[0] - 1);

            if(((rand[0] + 1) % (MW.MAPSIZE[0] - 2)) != 0)
                rand.push(rand[0] + 1);
            
            if(rand[0] > (MW.MAPSIZE[0] - 2))
                rand.push(rand[0] - MW.MAPSIZE[0] + 2);

            if(rand[0] < (MW.MAPSIZE[0] - 2)*(MW.MAPSIZE[1] - 3))
                rand.push(rand[0] + MW.MAPSIZE[0] - 2);
            ...
            barrier.pos[0] = rand[0]%(MW.MAPSIZE[0] - 2) + 1;
            barrier.pos[1] = Math.floor(rand[0]/(MW.MAPSIZE[0] - 2)) + 1;
    ...
    findPath: function(){
        var gr = new Array(MW.MAPSIZE[1]);
        for(var i = 0; i < gr.length; i++){
            gr[i] = [];
            for(var j = 0; j < MW.MAPSIZE[0]; j++){
                gr[i][j] = 1;
        ...
        var start = graph.grid[MW.MAPSIZE[0] - 1][MW.MAPSIZE[1] - 1];
    ...
    findShotPos: function(){
        ...
        if(minDis == 100000) return this.getPos(MW.MAPSIZE[0]- 1, MW.MAPSIZE[1] - 1);
        cc.log(minX + " " + minY);
    ...
    destroy: function(){
        this.stopAllActions();
        for(var i = 0; i < MW.CONTAINER.ENEMIES.length; i++) {
            MW.CONTAINER.ENEMIES[i].destroy();
            MW.CONTAINER.ENEMIES[i].stopAllActions();
        }
        for(var i = 0; i < MW.CONTAINER.BARRIERS.length; i++) {
            MW.CONTAINER.BARRIERS[i].destroy();

        }
        MW.CONTAINER.BARRIERS = [];
